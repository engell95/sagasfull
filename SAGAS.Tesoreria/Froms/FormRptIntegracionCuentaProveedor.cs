...
{
    public partial class FormRptIntegracionCuentaProveedor : Form
    {
        ...
        */
        ...
        public FormRptIntegracionCuentaProveedor()
        {
            InitializeComponent();
            XRTablaDatos();
            FillControl();
        ...
        private void bgwOn_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                //var id_proveedore = (from c in db.Proveedors
                //                     join t in db.TipoProveedors on c.TipoProveedorID equals t.ID
                //                     select new { c.ID }).ToList();

                if (lkTipoCta.ItemIndex != 0)
                {
                    
                    //id_proveedore = (from c in db.Proveedors
                    //                 //join t in db.TipoProveedors on c.TipoProveedorID equals t.ID
                    //                 where c.CuentaContableID.Equals((int)lkTipoCta.EditValue)
                    //                 select new { c.ID }).ToList();
                    ListaCL.AddRange(db.Proveedors.Where(o => o.CuentaContableID.Equals((int)lkTipoCta.EditValue)).Select(s => s.ID));
                }
                else if ((int)lkTipoCta.EditValue == 0)
                {
                    ListaCL.AddRange(db.Proveedors.Select(s => s.ID));
                }

                //if (id_proveedore.Equals(0))
                //    ListaCL.AddRange(id_proveedore);
                //else
                //    ListaCL.Add(id_proveedore);

                int z = ListaCL.Count;//id_proveedore.Count;
                decimal x = 0;

                bool SUS = (lkSES.EditValue == null ? false : true);

                EtVista = (from vcp in dv.VistaEstadoCuentaProveedor
                           where /*id_proveedore.Select(s => s.ID)*/ListaCL.Contains(vcp.ID) && !vcp.EsAnticipo && !vcp.MovimientoTipoID.Equals(61)
                            && ( vcp.FechaRegistro.Date >= Convert.ToDateTime(dateInicial.EditValue).Date && vcp.FechaRegistro.Date <= Convert.ToDateTime(dateFinal.EditValue).Date)
                ...
                List<Entidad.VistaEstadoCuentaProveedor> vistaMasivo = (from vcp in dv.VistaEstadoCuentaProveedorPagoMasivos
                                                                        where /*id_proveedore.Select(s => s.ID)*/ListaCL.Contains(vcp.ID)
                        && (Convert.ToDateTime(vcp.FechaRegistro).Date >= Convert.ToDateTime(dateInicial.EditValue).Date && Convert.ToDateTime(vcp.FechaRegistro).Date <= Convert.ToDateTime(dateFinal.EditValue).Date) 
                        && (vcp.EstacionServicioID.Equals(Convert.ToInt32(lkES.EditValue)) || Convert.ToInt32(lkES.EditValue).Equals(0))
                        && (!SUS || (SUS && ((vcp.SubEstacionID.Equals(Convert.ToInt32(lkSES.EditValue)) || Convert.ToInt32(lkSES.EditValue).Equals(0)))))
                ...
                var lSaldo = from o in EtVista
                             where ListaCL.Contains(o.ID)
                                 /*o.ID.Equals(id_proveedore)*/
                             //&& o.FechaRegistro.Date < Convert.ToDateTime(dateInicial.EditValue).Date
                             group o by new { o.ID } into gr
                             select new { ProveedorID = gr.Key.ID, Monto = gr.Where(o => o.FechaRegistro.Date <= Convert.ToDateTime(dateInicial.EditValue).Date).Sum(s => s.Monto) };

                decimal vSaldo = 0;
                if (lSaldo.Count() > 0)
                    vSaldo = Convert.ToDecimal(lSaldo.Sum(s => s.Monto));

                var Lista = EtVista.Where(o => ListaCL.Contains(o.ID)/*o.ID.Equals(id_proveedore)*/  && o.FechaRegistro.Date >= Convert.ToDateTime(dateInicial.EditValue).Date).OrderBy(o => o.FechaRegistro).ToList();
                /*******************************************************/
                x++;
                bgwOn.ReportProgress((int)Math.Ceiling((x / z) * 100));

                if (Lista.Count == 0 && vSaldo.Equals(0)) { Parametros.General.splashScreenManagerMain.CloseWaitForm(); return; }
                ///////////////////////////////////////////////////////////

                lSaldo.GroupBy(g => g.ProveedorID).ToList().ForEach(det =>
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                if (lkSES.EditValue.Equals(0))
                                    Estaciones = "Todas las Sub Estaciones";
                                else
                                    Estaciones = Convert.ToString(lkSES.Text);
                            }
                        }

                        var rLista = Lista.Where(o => o.ID.Equals(det.Key)).ToList();
                        var infoLista = EtVista.Where(o => o.ID.Equals(det.Key)).ToList();
                        ...
                        xrtbDatos.Rows[1].Cells[1].Text = (infoLista.Count() > 0 ? infoLista.First().Proveedores : "");
                        xrtbDatos.Rows[2].Cells[1].Text = (infoLista.Count() > 0 ? infoLista.First().RUC : "");
                        xrtbDatos.Rows[3].Cells[1].Text = (infoLista.Count() > 0 ? infoLista.First().Direccion : "");
                        xrtbDatos.Rows[4].Cells[1].Text = (infoLista.Count() > 0 ? infoLista.First().Telefonos : "");
                        ...
                        var rSaldo = lSaldo.SingleOrDefault(s => s.ProveedorID.Equals(det.Key));

                        xrSubDetalle.ReportSource = new SAGAS.Reportes.Tesoreria.Hojas.SubRptECProveedorMov(rLista, vFecha.ToString(), (rSaldo != null ? Convert.ToDecimal(rSaldo.Monto) : 0m), false);
                        ...
                        _SaldoTotal += Decimal.Round((rSaldo != null ? Convert.ToDecimal(rSaldo.Monto) : 0m) + Convert.ToDecimal(rLista.Sum(s => s.Monto)), 2, MidpointRounding.AwayFromZero);
        ...
        private void bgwOff_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                */
                ...
                ListaPR.AddRange(_ListProveedor.Where(o => !o.ID.Equals(0)).Select(s => s.ID));
        ...
        private void FormRptEstadoProveedor_Load(object sender, EventArgs e)
        {
            ...
            layoutControlItem5.Text = "Tipo Cuenta Proveedor";
        }
