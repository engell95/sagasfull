...
{
    public partial class FormSerieRetenciones : Parametros.Forms.FormBase
    {
        #region <<< DECLARACIONES >>>

        private Forms.Dialogs.DialogSerieRetenciones nf;
        ...
        private void linqInstantFeedbackSource1_GetQueryable(object sender, GetQueryableEventArgs e)
        {
            try
            {
                ...
                var query = from v in dv.VistaSeriesRetenciones
                            select new
                            {
                                v.Automatico,
                                v.EsAlcaldia,
        ...
        protected override void Add()
        {
            try
            {
                if (nf == null)
                {
                    nf = new Forms.Dialogs.DialogSerieRetenciones(Usuario);
                    nf.Text = "Nueva Serie Retenciones";
                    nf.Owner = this;
                    nf.MDI = this;
                    nf.Show();
                }
        ...
        protected override void Edit()
        {
            try
            {
                if (nf == null)
                {
                    ...
                    {
                        nf = new Forms.Dialogs.DialogSerieRetenciones(Usuario);
                        nf.Text = "Editar Serie Retenciones";
                        nf.EntidadAnterior = db.SeriesRetenciones.Single(e => e.ID.Equals(Convert.ToInt32(gvData.GetFocusedRowCellValue(colID))));
                        nf.Owner = this;
                        nf.Editable = true;
                        nf.MDI = this;
                        nf.Show();
            ...
            try
            {
                if (nf == null)
                {
                    if (gvData.FocusedRowHandle >= 0)
                    {
                        ...
                        {
                            var S = db.SeriesRetenciones.Single(c => c.ID == id);

                            //Eliminar Registro 
                            string texto = Convert.ToString(gvData.GetFocusedRowCellValue(gridColumn1));
                            db.SeriesRetenciones.DeleteOnSubmit(S);
                            db.SubmitChanges();

                            Parametros.General.AddLogBook(db, Parametros.TipoAccion.Tesoreria, "Se Eliminó la Serie: " + texto, this.Name);
