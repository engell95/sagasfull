...
{                                
    public partial class FormTipoAusencia : Parametros.Forms.FormBase
    {
        #region <<< DECLARACIONES >>>

        private Forms.Dialogs.DialogTipoAusencia nf;
        ...
        protected override void FillControl()
        {
            try
            {
                ...
                bdsManejadorDatos.DataSource = from P in db.TipoAusencia
                                               select P;
        ...
        protected override void Add()
        {
            try
            {
                if (nf == null)
                {
                    nf = new Forms.Dialogs.DialogTipoAusencia(Usuario);
                    nf.Text = "Crear Tipo de Ausencia";
                    nf.Owner = this;
                    nf.MDI = this;
                    nf.Show();
                }
        ...
        protected override void Edit()
        {
            try
            {
                if (nf == null)
                {
                    ...
                    {
                        nf = new Forms.Dialogs.DialogTipoAusencia(Usuario);
                        nf.Text = "Editar Tipo de Ausencia";
                        nf.EtAnterior = db.TipoAusencia.Single(e => e.ID == Convert.ToInt32(gvData.GetFocusedRowCellValue(colID)));
                        nf.Owner = this;
                        nf.Editable = true;
                        nf.MDI = this;
                        nf.Show();
            ...
            try
            {
                if (nf == null)
                {
                    if (gvData.FocusedRowHandle >= 0)
                    {
                        ...
                        var TP = db.TipoAusencia.Single(c => c.ID == id);
                        ...
                        if (gvData.GetFocusedRowCellValue(colActivo).Equals(true))
                        {
                            ...
                            {
                                if (Convert.ToInt32(db.Empleados.Where(o => o.ProfesionOficioID.Equals(TP.ID) && o.Activo).Count()) > 0)
                                {
                                ...
                                Parametros.General.AddLogBook(db, Parametros.TipoAccion.Nomina, "Se Inactivó el Tipo de ausencia: " + TP.Nombre, this.Name);
                        ...
                        else
                        {
                            if (Parametros.General.DialogMsg(Parametros.Properties.Resources.SEGUROACTIVAR + Environment.NewLine, Parametros.MsgType.question) == DialogResult.OK)
                            {
                                ...
                                Parametros.General.AddLogBook(db, Parametros.TipoAccion.Nomina, "Se activó el Tipo de ausencia: " + TP.Nombre, this.Name);
                                this.btnAnular.Caption = "Inactivar";
