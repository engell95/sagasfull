...
{                                
    public partial class FormProfesion : Parametros.Forms.FormBase
    {
        #region <<< DECLARACIONES >>>

        private Forms.Dialogs.DialogProfesion nf;
        ...
        protected override void FillControl()
        {
            try
            {
                ...
                bdsManejadorDatos.DataSource = from P in db.ProfesionOficio
                                               select P;
        ...
        protected override void Add()
        {
            try
            {
                if (nf == null)
                {
                    nf = new Forms.Dialogs.DialogProfesion(Usuario);
                    nf.Text = "Crear Profesion";
                    nf.Owner = this;
                    nf.MDI = this;
                    nf.Show();
                }
        ...
        protected override void Edit()
        {
            try
            {
                if (nf == null)
                {
                    ...
                    {
                        nf = new Forms.Dialogs.DialogProfesion(Usuario);
                        nf.Text = "Editar Profesion";
                        nf.EtAnterior = db.ProfesionOficio.Single(e => e.ID == Convert.ToInt32(gvData.GetFocusedRowCellValue(colID)));
                        nf.Owner = this;
                        nf.Editable = true;
                        nf.MDI = this;
                        nf.Show();
                ...
                try
                {
                    if (nf == null)
                    {
                        if (gvData.FocusedRowHandle >= 0)
                        {
                            ...
                            var P = db.ProfesionOficio.Single(c => c.ID == id);
                            ...
                            if (gvData.GetFocusedRowCellValue(colActivo).Equals(true))
                            {
                                if (Parametros.General.DialogMsg(Parametros.Properties.Resources.SEGUROINACTIVAR + Environment.NewLine, Parametros.MsgType.question) == DialogResult.OK)
                                {
                                    ...
                                    if (Convert.ToInt32(db.Empleados.Where(o => o.ProfesionOficioID.Equals(P.ID) && o.Activo).Count()) > 0)
                                    {
                                    ...
                                    Parametros.General.AddLogBook(db, Parametros.TipoAccion.Nomina, "Se Inactivó la Profesión: " + P.Nombre, this.Name);
                            ...
                            else
                            {
                                if (Parametros.General.DialogMsg(Parametros.Properties.Resources.SEGUROACTIVAR + Environment.NewLine, Parametros.MsgType.question) == DialogResult.OK)
                                {
                                    ...
                                    Parametros.General.AddLogBook(db, Parametros.TipoAccion.Nomina, "Se activó la Profesión: " + P.Nombre, this.Name);
                                    this.btnAnular.Caption = "Inactivar";
