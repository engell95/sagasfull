...
{
    public partial class FormSalidaManejo : Parametros.Forms.FormBase
    {
        #region <<< DECLARACIONES >>>

        private Forms.Dialogs.DialogSalidaManejo nf;
        ...
        private void linqInstantFeedbackSource1_GetQueryable(object sender, GetQueryableEventArgs e)
        {
            try
            {
                ...
                var query = from iv in dv.VistaMovimientos.Where(v => v.MovimientoTipoID.Equals(19) || v.MovimientoTipoID.Equals(21))
        ...
        private void ImprimirComprobante(int ID, bool ToPrint)
        {
            ...
            try
            {
                ...
                if (RD != null)
                {
                    rep.xrTableCellRD.Visible = true;
                    rep.xrTableCellRDNro.Visible = true;
            ...
            using (System.Data.Common.DbTransaction trans = db.Connection.BeginTransaction())
            {
                ...
                try
                {
                    ...
                    Parametros.General.AddLogBook(db, Parametros.TipoAccion.Inventario,
                                "Se anuló la Salida Manejo: " + Manterior.Referencia, this.Name);
                    ...
                    foreach (var dk in Manterior.Kardexes)
                    {
                        ...
                        Entidad.Deudor D = db.Deudors.Where(d => d.ClienteID.Equals(M.ClienteID) && d.Valor.Equals(-Math.Abs(KX.CantidadEntrada)) && d.ProductoID.Equals(KX.ProductoID) && d.MovimientoID.Equals(ID)).FirstOrDefault();
        ...
        protected override void Add()
        {
            try
            {
                if (nf == null)
                {
                    nf = new Forms.Dialogs.DialogSalidaManejo(Usuario, false);
                    nf.Text = "Nueva Salida Manejo";
                    nf.Owner = this.Owner;
                    ...
                    if (gvData.FocusedRowHandle >= 0)
                    {
                        if (Convert.ToInt32(gvData.GetFocusedRowCellValue("EstacionServicioID")).Equals(Parametros.General.EstacionServicioID))
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    nf = new Forms.Dialogs.DialogSalidaManejo(Usuario, true);
        ...
        protected override void Del()
        {
            if (gvData.FocusedRowHandle >= 0)
            {
                if (Convert.ToBoolean(gvData.GetFocusedRowCellValue(colAnulado)) || Convert.ToInt32(gvData.GetFocusedRowCellValue(colTipoMovimiento)).Equals(21))
        ...
        private void gvData_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                if (gvData.FocusedRowHandle >= 0)
                {
                    ...
                    if (M.MovimientoTipoID.Equals(18) || M.MovimientoTipoID.Equals(21))
                    {
                    ...
                    else if (M.MovimientoTipoID.Equals(19) || M.MovimientoTipoID.Equals(20))
                    {
