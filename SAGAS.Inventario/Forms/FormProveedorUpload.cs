...
{                                
    public partial class FormProveedorUpload : Parametros.Forms.FormBase
    {
        #region <<< DECLARACIONES >>>

        private Forms.Dialogs.DialogProveedorUpload nf;
        ...
        private void linqInstantFeedbackSource1_GetQueryable(object sender, GetQueryableEventArgs e)
        {
            try
            {
                ...
                var query = from iv in dv.VistaPlantilasInicales
                            where lista.Contains(iv.EstacionServicioID) &&iv.EsProveedor
                            select new
                            {
                                iv.ID,
                                iv.Numero,
        ...
        protected override void Imprimir()
        {
            try
            {
                if (gvData.FocusedRowHandle >= 0)
                {
                    ...
                    var VM = dbView.VistaPlantilasInicales.Where(m => m.ID.Equals(Convert.ToInt32(gvData.GetFocusedRowCellValue(colID)))).ToList();
                    Reportes.FormReportViewer rv = new Reportes.FormReportViewer();
                    rv.Text = "Carga Saldos Proveedores " + VM.First().Numero;

                    Reportes.Inventario.Hojas.RptProveedoresUpload rep = new Reportes.Inventario.Hojas.RptProveedoresUpload();
        ...
        protected override void Add()
        {
            try
            {
                if (nf == null)
                {
                    nf = new Forms.Dialogs.DialogProveedorUpload(Usuario, false);
                    nf.Text = "Crear Plantilla Proveedor";
                    nf.Owner = this.Owner;
            ...
            try
            {
                if (nf == null)
                {
                    if (gvData.FocusedRowHandle >= 0)
                    {
                        ...
                        {
                            nf = new Forms.Dialogs.DialogProveedorUpload(Usuario, true);
                            nf.Text = "Editar Plantilla Proveedor";
