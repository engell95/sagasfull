...
{                                
    public partial class FormArea : Parametros.Forms.FormBase
    {
        #region <<< DECLARACIONES >>>

        private Forms.Dialogs.DialogArea nf;
        ...
        protected override void FillControl()
        {
            try
            {
                ...
                bdsManejadorDatos.DataSource = from A in db.Areas
                                               join ct in db.CentroCostos on A.CentroCostoID equals ct.ID into DefaultCosto
                                               ...
                                               A.Comentario,
                                               A.Activo,
                                               CuentaVenta = (A.CuentaVentaID > 0 ? cuentas.Where(c => c.ID.Equals(A.CuentaVentaID)).First().Display : "< N/A >"),
                                               CuentaInventario = (A.CuentaInventarioID > 0 ? cuentas.Where(c => c.ID.Equals(A.CuentaInventarioID)).First().Display : "< N/A >"),
                                               CuentaCosto = (A.CuentaCostoID > 0 ? cuentas.Where(c => c.ID.Equals(A.CuentaCostoID)).First().Display : "< N/A >"),
                                               CuentaDescuento = (A.CuentaDescuentoID > 0 ? cuentas.Where(c => c.ID.Equals(A.CuentaDescuentoID)).First().Display : "< N/A >"),
                                               Costo = centro.Codigo + " " + centro.Nombre,
                                               CuentaSobrante = (A.CuentaSobranteID > 0 ? cuentas.Where(c => c.ID.Equals(A.CuentaSobranteID)).First().Display : "< N/A >"),
                                               CuentaFaltante = (A.CuentaFaltanteID > 0 ? cuentas.Where(c => c.ID.Equals(A.CuentaFaltanteID)).First().Display : "< N/A >")
                                               };
        ...
        protected override void Add()
        {
            try
            {
                if (nf == null)
                {
                    nf = new Forms.Dialogs.DialogArea();
                    nf.Text = "Crear Área";
                    nf.Owner = this;
                    nf.MDI = this;
                    nf.Show();
            ...
            try
            {
                if (nf == null)
                {
                    if (gvData.FocusedRowHandle >= 0)
                    {
                        nf = new Forms.Dialogs.DialogArea();
                        nf.Text = "Editar Área";
                        nf.EntidadAnterior = db.Areas.Single(e => e.ID == Convert.ToInt32(gvData.GetFocusedRowCellValue(colID)));
                        nf.Owner = this;
                        nf.Editable = true;
                        nf.MDI = this;
                        nf.Show();
                ...
                if (nf == null)
                {
                    if (gvData.FocusedRowHandle >= 0)
                    {
                        ...
                        if (gvData.GetFocusedRowCellValue(colActivo).Equals(true))
                        {
                            ...
                            {
                                if (db.ProductoClases.Count(o => o.AreaID.Equals(A.ID) && o.Activo) > 0)
                                {
                        ...
                        else
                        {
                            if (Parametros.General.DialogMsg(Parametros.Properties.Resources.SEGUROACTIVAR + Environment.NewLine, Parametros.MsgType.question) == DialogResult.OK)
                            {
                                ...
                                Parametros.General.AddLogBook(db, Parametros.TipoAccion.Administrativa, "Se activó el Área: " + A.Nombre, this.Name);
                                this.btnAnular.Caption = "Inactivar";
