...
{                                
    public partial class FormUnidadMedida : Parametros.Forms.FormBase
    {
        #region <<< DECLARACIONES >>>

        private Forms.Dialogs.DialogUnidadMedida nf;
        ...
        protected override void FillControl()
        {
            try
            {
                ...
                bdsManejadorDatos.DataSource = from P in db.UnidadMedidas
                                               select P;
        ...
        protected override void Add()
        {
            try
            {
                if (nf == null)
                {
                    nf = new Forms.Dialogs.DialogUnidadMedida();
                    nf.Text = "Crear Unidad de Medida";
                    nf.Owner = this;
                    nf.MDI = this;
                    nf.Show();
                }
        ...
        protected override void Edit()
        {
            try
            {
                if (nf == null)
                {
                    ...
                    {
                        nf = new Forms.Dialogs.DialogUnidadMedida();
                        nf.Text = "Editar Unidad de Medida";
                        nf.EntidadAnterior = db.UnidadMedidas.Single(e => e.ID == Convert.ToInt32(gvData.GetFocusedRowCellValue(colID)));
                        nf.Owner = this;
                        nf.Editable = true;
                        nf.MDI = this;
                        nf.Show();
            ...
            try
            {
                if (nf == null)
                {
                    if (gvData.FocusedRowHandle >= 0)
                    {
                        ...
                        var UM = db.UnidadMedidas.Single(c => c.ID == id);
                        ...
                        if (gvData.GetFocusedRowCellValue(colActivo).Equals(true))
                        {
                            if (Parametros.General.DialogMsg(Parametros.Properties.Resources.SEGUROINACTIVAR + Environment.NewLine, Parametros.MsgType.question) == DialogResult.OK)
                            {
                                ...
                                Parametros.General.AddLogBook(db, Parametros.TipoAccion.Administrativa, "Se Inactivó la Unidad de Medida: " + UM.Nombre, this.Name);
                        ...
                        else
                        {
                            if (Parametros.General.DialogMsg(Parametros.Properties.Resources.SEGUROACTIVAR + Environment.NewLine, Parametros.MsgType.question) == DialogResult.OK)
                            {
                                ...
                                Parametros.General.AddLogBook(db, Parametros.TipoAccion.Administrativa, "Se activó la Unidad de Medida: " + UM.Nombre, this.Name);
                                this.btnAnular.Caption = "Inactivar";
