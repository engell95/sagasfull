...
{
    public partial class FormActivo : Parametros.Forms.FormBase
    {
        #region <<< DECLARACIONES >>>

        private Forms.Dialogs.DialogActivo nf;
        ...
        protected override void FillControl()
        {
            try
            {
                ...
                bdsManejadorDatos.DataSource = from A in db.Activo
                                               join TA in db.TipoActivo on A.TipoActivoID equals TA.ID
                                               select new
                                               {
                                                   A.ID,
                                                   A.Codigo,
        ...
        protected override void Add()
        {
            try
            {
                if (nf == null)
                {
                    nf = new Forms.Dialogs.DialogActivo();
                    nf.Text = "Crear Activo";
                    nf.Owner = this;
                    nf.MDI = this;
                    nf.Show();
                }
        ...
        protected override void Edit()
        {
            try
            {
                if (nf == null)
                {
                    ...
                    {
                        nf = new Forms.Dialogs.DialogActivo();
                        nf.Text = "Editar Activo";
                        nf.EntidadAnterior = db.Activo.Single(e => e.ID == Convert.ToInt32(gvData.GetFocusedRowCellValue(colID)));
                        nf.Owner = this;
                        nf.Editable = true;
                        nf.MDI = this;
                        nf.Show();
            ...
            try
            {
                if (nf == null)
                {
                    if (gvData.FocusedRowHandle >= 0)
                    {
                        ...
                        var A = db.Activo.Single(c => c.ID == id);
                        ...
                        if (gvData.GetFocusedRowCellValue(colActivo).Equals(true))
                        {
                            if (Parametros.General.DialogMsg(Parametros.Properties.Resources.SEGUROINACTIVAR + Environment.NewLine, Parametros.MsgType.question) == DialogResult.OK)
                            {
                                ...
                                {
                                    Parametros.General.DialogMsg("El Activo esta asignado en el sistema", Parametros.MsgType.warning);
                                    return;
                                }
                                //Desactivar Registro 
                                A.Activado = false;
                                ...
                                Parametros.General.AddLogBook(db, Parametros.TipoAccion.ActivoFijo, "Se Inactivó el Activo: " + A.Nombre, this.Name);
                        ...
                        else
                        {
                            if (Parametros.General.DialogMsg(Parametros.Properties.Resources.SEGUROACTIVAR + Environment.NewLine, Parametros.MsgType.question) == DialogResult.OK)
                            {
                                ...
                                Parametros.General.AddLogBook(db, Parametros.TipoAccion.ActivoFijo, "Se activó el Activo: " + A.Nombre, this.Name);
                                this.btnAnular.Caption = "Inactivar";
