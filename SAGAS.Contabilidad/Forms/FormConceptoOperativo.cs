...
{                                
    public partial class FormConceptoOperativo : Parametros.Forms.FormBase
    {
        #region <<< DECLARACIONES >>>

        private Forms.Dialogs.DialogConceptoOperativo nf;
        ...
        protected override void FillControl()
        {
            try
            {
                ...
                bdsManejadorDatos.DataSource = from cto in db.ConceptoCajaOperativas                                           
                                               select cto;
                ...
                lkMoneda.DataSource = db.Monedas.Select(s => new { s.ID, Display = s.Simbolo + " | " + s.Nombre }).ToList();
        ...
        protected override void Add()
        {
            try
            {
                if (nf == null)
                {
                    nf = new Forms.Dialogs.DialogConceptoOperativo();
                    nf.Text = "Crear Concepto Operativo";
                    nf.Owner = this;                     
                    nf.MDI = this;
                    nf.Show();
                }
        ...
        protected override void Edit()
        {
            try
            {
                if (nf == null)
                {
                    ...
                    {
                        nf = new Forms.Dialogs.DialogConceptoOperativo();
                        nf.Text = "Editar Concepto Operativo";
                        nf.EntidadAnterior = db.ConceptoCajaOperativas.Single(e => e.ID.Equals(Convert.ToInt32(gvData.GetFocusedRowCellValue(colID))));
                        nf.Owner = this;
                        nf.Editable = true;
                        nf.MDI = this;
                        nf.Show();
            ...
            try
            {
                if (nf == null)
                {
                    if (gvData.FocusedRowHandle >= 0)
                    {
                        ...
                        var C = db.ConceptoCajaOperativas.Single(c => c.ID == id);
