...
{                                
    public partial class FormCierrePeriodoFiscal : Parametros.Forms.FormBase
    {
        #region <<< DECLARACIONES >>>

        private Forms.Dialogs.DialogCierrePeriodoFiscal nf;
        ...
        public FormCierrePeriodoFiscal()
        {
            InitializeComponent();
        ...
        private void linqInstantFeedbackSource1_GetQueryable(object sender, GetQueryableEventArgs e)
        {
            try
            {
                ...
                var query = from iv in dv.VistaMovimientos.Where(v => v.MovimientoTipoID.Equals(64) || v.MovimientoTipoID.Equals(65))
        ...
        private void ImprimirComprobante(int ID, bool ToPrint)
        {
            ...
            try
            {
                ...
                rep.CeTitulo.Text = VM.First().MovimientoTipoNombre + "  " + ((VM.First().Anulado || VM.First().MovimientoTipoID.Equals(65)) ? VM.First().Referencia : VM.First().Numero.ToString("000000000") + "  " + (VM.First().Modificado ? "MODIFICADO" : ""));
                rep.CeEstacion.Text = (VM.First().SubEstacionID > 0 ? VM.First().SubEstacionNombre : VM.First().EstacionNombre);

                if (VM.First().Anulado || VM.First().MovimientoTipoID.Equals(65))
        ...
        protected override void Add()
        {
            try
            {
                if (nf == null)
                {
                    nf = new Forms.Dialogs.DialogCierrePeriodoFiscal(Usuario, false);
                    nf.Text = "Nuevo Cierre Período Fiscal";
                    nf.Owner = this.Owner;
            ...
            using (System.Data.Common.DbTransaction trans = db.Connection.BeginTransaction())
            {
                ...
                try
                {
                    ...
                    Parametros.General.AddLogBook(db, Parametros.TipoAccion.Contabilidad,
                        "Se anuló el comprobante de cierre de periodo fiscal : " + Manterior.Numero.ToString("000000000"), this.Name);
        ...
        protected override void Edit()
        {
            ...
            //                if (MAnterior.SedeID.Equals(Parametros.General.SedeID))
            //                {
            //                    if (Parametros.General.ValidatePeriodoContable(MAnterior.FechaRegistro, db, MAnterior.SedeID))
            //                    {
            //                        nf = new Forms.Dialogs.DialogCierrePeriodoFiscal(Usuario, true);
            //                        nf.Text = "Editar Cierre Período Fiscal";
            ...
            //                    else
            //                    {
            //                        Parametros.General.DialogMsg("El Periodo Fiscal para esta fecha ya esta cerrado.", Parametros.MsgType.warning);
            //                    }
            //                }
            //                else
            //                {
            //                    Parametros.General.DialogMsg("La Estación de Servicio Seleccionada no es la misma que la Estación de Servicio del Cierre Período Fiscal.", Parametros.MsgType.warning);
            //                }
            //            }
            //            else
            //                nf.Activate();
