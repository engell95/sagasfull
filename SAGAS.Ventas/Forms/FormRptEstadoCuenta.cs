...
{
    public partial class FormRptEstadoClientes : Form
    {
        ...
        public decimal _Limite, _Saldo, _Disponible;
        public decimal Limite 
        {
            get { return Convert.ToDecimal(spLimite.EditValue); }
            set { spLimite.EditValue = value; }
        }
        public decimal Saldo
        {
            get { return Convert.ToDecimal(spSaldo.EditValue); }
            set { spSaldo.EditValue = value; }
        }
        public decimal Disponible
        {
            get { return Convert.ToDecimal(spDisponible.EditValue); }
            set { spDisponible.EditValue = value; }
        }
        ...
        private void FillControl()
        {
            try
            {
                ...
                _ListClientes = GetListClientesbyES(Convert.ToInt32(lkES.EditValue));
        ...
        private List<ListaClientesCuentas> GetListClientesbyES(int EstacionServicioID)
        {
            return (from m in db.Movimientos
                    join c in db.Clientes on m.ClienteID equals c.ID
                    where m.ClienteID > 0 && (m.EstacionServicioID.Equals(EstacionServicioID) || EstacionServicioID == 0) && c.TipoClienteID != 4 //&& c.CuentaContableID == Convert.ToInt32(lkTipoCta.EditValue)
                    ...
                    select new ListaClientesCuentas 
                    {
                        ...
                        Codigo = g.Key.Codigo,
                        Nombre = g.Key.Nombre,
                        Display = g.Key.Codigo + " | " + g.Key.Nombre,
        ...
        private void lkES_EditValueChanged(object sender, EventArgs e)
        {
            if (lkES.EditValue != null)
            {
                ...
                {
                    #region :::Ocultando las Subestaciones:::
                ...
                else if (!Convert.ToInt32(lkES.EditValue).Equals(0))
                {
                    ...
                    if (ES != null)
                    {
                        ...
                        if (Sus.Count > 0)
                        {
                            #region :::Si seleccionó una ES con SES:::
                        ...
                        {
                            #region :::Si seleccionó una ES sin SES:::
                            this.lkSES.EditValue = null;
                            this.lkSES.Properties.DataSource = null;
                            this.layoutControlItemSes.Visibility = DevExpress.XtraLayout.Utils.LayoutVisibility.Never;
                            #endregion
                ...
                _ListClientes = GetListClientesbyES(Convert.ToInt32(lkES.EditValue));
                lkTipoCta_EditValueChanged(null, null);
            }
            else
            {
                this.lkSES.EditValue = null;
            ...
            try
            {
                if (IDCliente > 0)
                {
                    if (!_EsEmpleado)
                    {
                        ...
                        //var obj = (from d in db.Deudors                                           //esto de abajo estaba solo con IDEstacionServicio
                        //           join m in db.Movimientos.Where(m => m.EstacionServicioID.Equals(lkES.EditValue != null ? Convert.ToInt32(lkES.EditValue) : IDEstacionServicio) && m.SubEstacionID.Equals(IDSubEstacion)) on d.MovimientoID equals m.ID
        ...
        private void bgwOn_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                if (_EsEmpleado)
                {
                    ...
                    if(!IDCliente.Equals(0)){
                        decimal saldototalAux =0;
                        ListaCL.ForEach(det =>
                        {
                            #region :::Recorrido C/Empleado SALDO TOTAL:::
                            //SAGAS.Reportes.Ventas.Hojas.RptECClienteMov Mrep = new SAGAS.Reportes.Ventas.Hojas.RptECClienteMov();

                            /************************************/


                            List<Entidad.VistaEstadoCuentaEmpleadoContabilidad> EtV = (from vcc in dv.VistaEstadoCuentaEmpleadoContabilidad
                                                                          where vcc.ID.Equals(det)
                                                                           && vcc.FechaRegistro.Date <= Convert.ToDateTime(dateFinal.EditValue).Date
                                                                          select vcc).ToList();

                            EtV.OrderBy(o => o.FechaRegistro);

                            var lSaldoAux = EtV.Where(o => o.ID.Equals(det) && o.FechaRegistro.Date < Convert.ToDateTime(dateInicial.EditValue).Date);

                            decimal vSaldo = 0;
                            if (lSaldoAux.Count() > 0)
                                vSaldo = Convert.ToDecimal(lSaldoAux.Sum(s => s.Monto));

                            var Lista = EtV.Where(o => o.ID.Equals(det) && o.FechaRegistro.Date >= Convert.ToDateTime(dateInicial.EditValue).Date).OrderBy(o => o.FechaRegistro).ToList();
                            /**********************************/
                            if (Lista.Count == 0 && vSaldo.Equals(0)) return;
                            ////////////////////////////////////

                            saldototalAux += Decimal.Round(vSaldo + Convert.ToDecimal(Lista.Sum(s => s.Monto)), 2, MidpointRounding.AwayFromZero);
                            _Saldo = Convert.ToDecimal(saldototalAux.ToString("#,0.00"));
                            #endregion //fin lista clientes Det
                        });
                    }
                    ListaCL.ForEach(det =>
                    {
                ...
                else
                {
                    ...
                    if (IDCliente.Equals(0))
                        ListaCL.AddRange(_ListClientes.Where(o => !o.ID.Equals(0) && o.CuentaID.Equals(Convert.ToInt32(lkTipoCta.EditValue))).Select(s => s.ID));
                    else
                        ListaCL.Add(IDCliente);
                    ...
                    //ListaCL.AddRange(_ListClientes.Where(o => !o.ID.Equals(0)).Select(s => s.ID));
                        ...
                        EtVista = (from vcc in dv.VistaEstadoCuentaCliente
                                   where ListaCL.Contains(vcc.ID)
                                    && vcc.FechaRegistro.Date <= Convert.ToDateTime(dateFinal.EditValue).Date
                                    && (vcc.EstacionServicioID.Equals(Convert.ToInt32(lkES.EditValue)) || Convert.ToInt32(lkES.EditValue).Equals(0))
                                    && (!SUS || (SUS && ((vcc.SubEstacionID.Equals(Convert.ToInt32(lkSES.EditValue)) || Convert.ToInt32(lkSES.EditValue).Equals(0)))))
                                    && vcc.CuentaContableID == Convert.ToInt32(lkTipoCta.EditValue)
                                   select vcc).ToList();

                        #region Cuentas de Caja (Moneda Nacional & Extranjera)
                        if ((ListaCL.Contains(_ContadoCordobas) && Convert.ToInt32(lkTipoCta.EditValue).Equals(4)) || (ListaCL.Contains(_ContadoDolares) && Convert.ToInt32(lkTipoCta.EditValue).Equals(5)))
                        {
                            int md = 0;
                            if (ListaCL.Contains(_ContadoCordobas))
                                md = _ContadoCordobas;
                            else
                                md = _ContadoDolares;

                            var CL = db.Clientes.Single(s => s.ID.Equals(md));
                            ...
                            List<Entidad.VistaEstadoCuentaCliente> obj = (from m in db.Movimientos
                                                                          ...
                                                                          where (m.MonedaID.Equals(md.Equals(_ContadoCordobas) ? 1 : 2) && (m.AreaID > 0))// || m.ClienteID.Equals(det))
                        ...
                        //}
                        #endregion
                        EtVista.GroupBy(g => g.ID).ToList().ForEach(xx =>
                        {
                        xx.OrderBy(o => o.FechaRegistro);

                        //var lSaldo =  xx.Where(o => o.ID.Equals(det) && o.FechaRegistro.Date < Convert.ToDateTime(dateInicial.EditValue).Date);
                        var lSaldo = xx.Where(o => o.FechaRegistro.Date < Convert.ToDateTime(dateInicial.EditValue).Date);

                        decimal vSaldo = 0;
                        if (lSaldo.Count() > 0)
                            vSaldo = Convert.ToDecimal(lSaldo.Sum(s => s.Monto));

                        //var Lista = xx.Where(o => o.ID.Equals(det) && o.FechaRegistro.Date >= Convert.ToDateTime(dateInicial.EditValue).Date).OrderBy(o => o.FechaRegistro).ToList();
                        var Lista = xx.Where(o => o.FechaRegistro.Date >= Convert.ToDateTime(dateInicial.EditValue).Date).OrderBy(o => o.FechaRegistro).ToList();
                        /**********************************/
                        if (Lista.Count == 0 && vSaldo.Equals(0)) return;
                        ////////////////////////////////////

                        #region Ejecución del Reporte
                        SAGAS.Reportes.Ventas.Hojas.RptECClienteMov Mrep = new SAGAS.Reportes.Ventas.Hojas.RptECClienteMov();
                        string Nombre, Direccion, Telefono;
                        System.Drawing.Image picture_LogoEmpresa;
                        Parametros.General.GetCompanyData(out Nombre, out Direccion, out Telefono, out picture_LogoEmpresa);
                        ...
                        xrtbDatos.StylePriority.UseTextAlignment = false;
                        xrtbDatos.Rows[0].Cells[1].Text = Estaciones;
                        xrtbDatos.Rows[1].Cells[1].Text = (xx.Count() > 0 ? xx.First().Clientes : "");
                        xrtbDatos.Rows[2].Cells[1].Text = (xx.Count() > 0 ? xx.First().RUC : "");
                        xrtbDatos.Rows[3].Cells[1].Text = (xx.Count() > 0 ? xx.First().Direccion : "");
                        xrtbDatos.Rows[4].Cells[1].Text = (xx.Count() > 0 ? xx.First().Telefonos : "");
                    ...
                    if (!IDCliente.Equals(0))
                    {
                        ...
                        ListaCL.ForEach(det =>
                        {
                            #region :::Recorrido C/Cliente SALDO TOTAL:::
                            //SAGAS.Reportes.Ventas.Hojas.RptECClienteMov Mrep = new SAGAS.Reportes.Ventas.Hojas.RptECClienteMov();

                            /************************************/
                            //bool SUS = (lkSES.EditValue == null ? false : true);

                            List<Entidad.VistaEstadoCuentaCliente> EtV = (from vcc in dv.VistaEstadoCuentaCliente
                                                                          where vcc.ID.Equals(det)
                                                                           && vcc.FechaRegistro.Date <= Convert.ToDateTime(dateFinal.EditValue).Date
                                                                              //&& Convert.ToInt32(lkES.EditValue).Equals(0)
                                                                              //&& (!SUS || Convert.ToInt32(lkSES.EditValue).Equals(0))
                                                                           && vcc.CuentaContableID == Convert.ToInt32(lkTipoCta.EditValue)
                                                                          select vcc).ToList();
                ...
                _Limite = (IDCliente.Equals(0) ? 0m : (_EsEmpleado ? 0m : Convert.ToDecimal(db.Clientes.Where(c => c.ID.Equals(IDCliente)).FirstOrDefault().LimiteCredito)));
                //_Saldo = Convert.ToDecimal(saldototalAux.ToString("#,0.00"));
                _Disponible = Convert.ToDecimal(_Limite) - Convert.ToDecimal(_Saldo);
                e.Result = MainMrep;
            ...
            try
            {
               Parametros.General.ShowWaitSplash(this, Parametros.WaitFormCaption.GENERANDO.ToString(), Parametros.Properties.Resources.TXTVISTAPREVIA);
                ...
                if (_EsEmpleado)
                {
                    ...
                    var ListaDAux=(from v in dbView.DocumentosPagadosEmpleado(Convert.ToDateTime(dateFinal.EditValue).Date)
                                  where ListaCL.Contains(v.EmpleadoID)
                                  select v).OrderBy(o => o.FechaContabilizacion).ToList();
                    #endregion
                    
                    #region :::Setting the Report:::
                    ...
                    if (!IDCliente.Equals(0))
                    {
                        decimal abono = Convert.ToDecimal(ListaDAux.Sum(d => d.Abono).ToString());
                        decimal monto = Convert.ToDecimal(ListaDAux.Sum(s => s.Monto).ToString());
                        _Limite = 0m;
                        _Saldo = monto - abono;
                        _Disponible = _Limite - _Saldo;
                    }
                ...
                else
                {
                    ...
                    //Llenando la lista de los clientes
                    if (IDCliente.Equals(0))
                        ListaCL.AddRange(_ListClientesToShow.Where(o => !o.ID.Equals(0) && o.CuentaID.Equals(Convert.ToInt32(lkTipoCta.EditValue))).Select(s => s.ID));
                    else
                        ListaCL.Add(IDCliente);

                    //ListaCL.ForEach(det =>
                    //{
                        //#region ::: Recorrido por c/Cliente :::
                        //if (!det.Equals(0))
                        //{
                        ...
                        if (lkTipoCta.EditValue != null)
                        {
                            ...
                            {
                                vCuenta = _ListClientesToShow.Single(p => p.ID.Equals(IDCliente)).CuentaID;
                            }
                        }
                        else
                        {
                        ...
                        //vCuenta = (lkTipoCta.EditValue != null ? Convert.ToInt32(lkTipoCta.EditValue) : 0); //_ListClientes.Single(p => p.ID.Equals(IDCliente)).CuentaID;
                        #endregion
                        bool SUS = (lkSES.EditValue == null ? false : true);

                        List<Entidad.MovimientosPagadosResult> ListaAux;
                        if (IDCliente.Equals(0))
                        {
                            ListaAux = (from v in dbView.MovimientosPagados(Convert.ToDateTime(dateFinal.EditValue).Date, vCuenta)
                        ...
                        {
                            ListaAux = (from v in dbView.MovimientosPagadosPorCliente(Convert.ToDateTime(dateFinal.EditValue).Date, vCuenta, IDCliente) 
                                        where (v.EstacionServicioID.Equals(Convert.ToInt32(lkES.EditValue)) || Convert.ToInt32(lkES.EditValue).Equals(0))
                                                                                && (!SUS || (SUS && ((v.SubEstacionID.Equals(Convert.ToInt32(lkSES.EditValue)) || Convert.ToInt32(lkSES.EditValue).Equals(0)))))
                        ...
                        List<Entidad.MovimientosPagadosResult> ListaRAux = (from v in dbView.MovimientosPagadosOrigenROC(Convert.ToDateTime(dateFinal.EditValue).Date, vCuenta)
                        ...
                        List<Entidad.MovimientosPagadosResult> ListaS = new List<Entidad.MovimientosPagadosResult>();
                        ListaS.AddRange(ListaAux);
                        ListaS.AddRange(ListaRAux);


                        if (!IDCliente.Equals(0))
                        {
                            decimal abono = Convert.ToDecimal(ListaS.Sum(d => d.Abono).ToString());
                            decimal monto = Convert.ToDecimal(ListaS.Sum(s => s.Monto).ToString());
                            _Limite = Convert.ToDecimal(db.Clientes.Where(c => c.ID.Equals(IDCliente)).FirstOrDefault().LimiteCredito);
                            _Saldo = monto - abono;
                            _Disponible = _Limite - _Saldo;
                        }
                        #region :::DS del reporte:::
                        //PENDIENTES AL CORTE                                
                        List<Entidad.MovimientosPagadosResult> ListaD = (from v in ListaAux
                                                                         where (v.EstacionServicioID.Equals(Convert.ToInt32(lkES.EditValue)) || Convert.ToInt32(lkES.EditValue).Equals(0))
                                                                         && (SUS ? (v.SubEstacionID.Equals(Convert.ToInt32(lkSES.EditValue)) || Convert.ToInt32(lkSES.EditValue).Equals(0)) : SUS.Equals(false))
                                                                         //EstacionServicioID.Equals(Convert.ToInt32(lkES.EditValue)) || Convert.ToInt32(lkES.EditValue).Equals(0))
                                                                         select v).OrderBy(o => o.FechaContabilizacion).ToList();



                        List<Entidad.MovimientosPagadosResult> ListaRoc = (from v in ListaRAux
                                                                           where (v.EstacionServicioID.Equals(Convert.ToInt32(lkES.EditValue)) || Convert.ToInt32(lkES.EditValue).Equals(0))
                                                                         //&& (SUS ? (v.SubEstacionID.Equals(Convert.ToInt32(lkSES.EditValue)) || Convert.ToInt32(lkSES.EditValue).Equals(0)) : SUS.Equals(false))
                                                                           select new Entidad.MovimientosPagadosResult
                                                                           {
        ...
        //Metodo Asincrono para cargar la lista
                ...
                if (Convert.ToInt32(LkValue).Equals(_CuentaEmpleadoID))
                {
                    ...
                    if (Convert.ToInt32(LkValue) > 0)
                    {
                        ...
                        _ListClientesToShow = (from em in db.Empleados
                                               ...
                                               where m.EstacionServicioID.Equals(Convert.ToInt32(lkES.EditValue))//IDEstacionServicio)
