...
{                                
    public partial class FormTipoCliente : Parametros.Forms.FormBase
    {
        #region <<< DECLARACIONES >>>

        private Forms.Dialogs.DialogTipoCliente nf;
        ...
        protected override void FillControl()
        {
            try
            {
                ...
                bdsManejadorDatos.DataSource = from tp in db.TipoClientes
                                               select tp;
        ...
        protected override void Add()
        {
            try
            {
                if (nf == null)
                {
                    nf = new Forms.Dialogs.DialogTipoCliente();
                    nf.Text = "Crear Tipo de CLientes";
                    nf.Owner = this;
                    nf.MDI = this;
                    nf.Show();
                }
        ...
        protected override void Edit()
        {
            try
            {
                if (nf == null)
                {
                    ...
                    {
                        nf = new Forms.Dialogs.DialogTipoCliente();
                        nf.Text = "Editar Tipo de Clientes";
                        nf.EntidadAnterior = db.TipoClientes.Single(e => e.ID == Convert.ToInt32(gvData.GetFocusedRowCellValue(colID)));
                        nf.Owner = this;
                        nf.Editable = true;
                        nf.MDI = this;
                        nf.Show();
            ...
            try
            {
                if (nf == null)
                {
                    if (gvData.FocusedRowHandle >= 0)
                    {
                        ...
                        var tp = db.TipoClientes.Single(c => c.ID == id);
                        ...
                        if (gvData.GetFocusedRowCellValue(colActivo).Equals(true))
                        {
                            if (Parametros.General.DialogMsg(Parametros.Properties.Resources.SEGUROINACTIVAR + Environment.NewLine, Parametros.MsgType.question) == DialogResult.OK)
                            {
                                ...
                                if (Convert.ToInt32(db.Clientes.Where(o => o.TipoClienteID.Equals(tp.ID) && o.Activo).Count()) > 0)
                                {
                                ...
                                if (tp.ID.Equals(Parametros.Config.TipoClienteManejoID()))
                                {
                                    Parametros.General.DialogMsg(Parametros.Properties.Resources.MSGNOPUEDEINACTIVAR + Environment.NewLine, Parametros.MsgType.warning);
                                    return;
                                }
                                ...
                                Parametros.General.AddLogBook(db, Parametros.TipoAccion.Administrativa, "Se Inactivó Tipo de Clientes: " + tp.Nombre, this.Name);
                        ...
                        else
                        {
                            if (Parametros.General.DialogMsg(Parametros.Properties.Resources.SEGUROACTIVAR + Environment.NewLine, Parametros.MsgType.question) == DialogResult.OK)
                            {
                                ...
                                Parametros.General.AddLogBook(db, Parametros.TipoAccion.Administrativa, "Se activó Tipo de Clientes: " + tp.Nombre, this.Name);
                                this.btnAnular.Caption = "Inactivar";
