...
{                                
    public partial class FormTanque : Parametros.Forms.FormBase
    {
        #region <<< DECLARACIONES >>>

        private Forms.Dialogs.DialogTanque nf;
        ...
        protected override void FillControl()
        {
            try
            {
                ...
                bdsManejadorDatos.DataSource = from T in db.Tanques
                                               where (db.GetViewEstacionesServicioByUsers.Any(ges => ges.UsuarioID == Usuario && ges.EstacionServicioID == T.EstacionServicioID))
                                               select new
                                               {
                                                   T.ID,
                ...
                //**TipoTanque
                lkrTipoTanque.DataSource = from tp in db.TipoTanques
                                            where tp.Activo
                                            select new { tp.ID, tp.Nombre };

                lkrTipoTanque.DisplayMember = "Nombre";
                lkrTipoTanque.ValueMember = "ID";
        ...
        protected override void Add()
        {
            try
            {
                if (nf == null)
                {
                    nf = new Forms.Dialogs.DialogTanque(Usuario, IDCombustible);
                    nf.Text = "Crear Tanque";
                    nf.Owner = this;
                    nf.MDI = this;
                    nf.Show();
                }
        ...
        protected override void Edit()
        {
            try
            {
                if (nf == null)
                {
                    ...
                    {
                        nf = new Forms.Dialogs.DialogTanque(Usuario, IDCombustible);
                        nf.Text = "Editar Tanque";
                        nf.EntidadAnterior = db.Tanques.Single(e => e.ID == Convert.ToInt32(gvData.GetFocusedRowCellValue(colID)));
                        nf.Owner = this;
                        nf.Editable = true;
                        nf.MDI = this;
                        nf.Show();
            ...
            try
            {
                if (nf == null)
                {
                    if (gvData.FocusedRowHandle >= 0)
                    {
                        ...
                        var T = db.Tanques.Single(c => c.ID == id);
                        ...
                        if (gvData.GetFocusedRowCellValue(colActivo).Equals(true))
                        {
                            if (Parametros.General.DialogMsg(Parametros.Properties.Resources.SEGUROINACTIVAR + Environment.NewLine, Parametros.MsgType.question) == DialogResult.OK)
                            {
                                ...
                                Parametros.General.AddLogBook(db, Parametros.TipoAccion.Arqueo, "Se Inactivó el Tanque: " + T.Nombre, this.Name);
                        ...
                        else
                        {
                            if (Parametros.General.DialogMsg(Parametros.Properties.Resources.SEGUROACTIVAR + Environment.NewLine, Parametros.MsgType.question) == DialogResult.OK)
                            {
                                ...
                                Parametros.General.AddLogBook(db, Parametros.TipoAccion.Arqueo, "Se activó el Tanque: " + T.Nombre, this.Name);
                                this.btnAnular.Caption = "Inactivar";
