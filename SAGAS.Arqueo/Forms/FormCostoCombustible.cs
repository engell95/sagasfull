...
{                                
    public partial class FormCostoCombustible : Parametros.Forms.FormBase
    {
        #region <<< DECLARACIONES >>>

        private Forms.Dialogs.DialogCostoCombustible nf;
        ...
        private void FormZona_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                this.btnCambiarRangoPrecio.Enabled = true; // Parametros.General.SystemOptionAcces(Parametros.General.UserID, "btnCambiarRangoPrecio");
            }
            catch (Exception ex)
            {
        ...
        protected override void FillControl() 
        {
            try
            {
                ...
                bdsManejadorDatos.DataSource = from pc in db.CostoCombustibles
                                               select pc;
        ...
        protected override void Add()
        {
            try
            {
                if (nf == null)
                {
                    nf = new Forms.Dialogs.DialogCostoCombustible();
                    nf.Text = "Crear costo de combustible";
                    nf.Owner = this;
                    nf.MDI = this;
                    nf.Show();
                }
        ...
        protected override void Edit()
        {
            try
            {
                if (nf == null)
                {
                    ...
                    {
                        nf = new Forms.Dialogs.DialogCostoCombustible();
                        nf.Text = "Editar costo de combustible";
                        nf.EntidadAnterior = db.CostoCombustibles.Single(e => e.ID == Convert.ToInt32(gvData.GetFocusedRowCellValue(colID)));
                        nf.Owner = this;
                        nf.Editable = true;
                        nf.MDI = this;
                        nf.Show();
                ...
                if (e.Field != null)
                {
                    if (e.Field.FieldName.Equals("PrecioDiferenciado"))
                    {
                        ...
                        if (e.Value.Equals(true))
                            e.DisplayText = "COSTO DIFERENCIADO (C$ por Galones)";
                        else
                            e.DisplayText = "COSTO FULL (C$ por Galones)";
                    }
                }
            }
            catch (Exception ex)
...
//if (e.IsColumn)
        ...
        private void gvData_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            if (gvData.FocusedRowHandle >= 0)
            {
                try
                {
                    ...
                    var obj = db.spListaCostoCombustible(Convert.ToInt32(gvData.GetFocusedRowCellValue(colID)));
        ...
        private void btnCambiarRangoPrecio_Click(object sender, EventArgs e)
        {
            try
            {
                    if (gvData.FocusedRowHandle >= 0)
                    {
                        Forms.Dialogs.DialogNuevoRangoCostoCombustible dg = new Forms.Dialogs.DialogNuevoRangoCostoCombustible();
                        dg.Text = "Editar Nuevo Rango para la Lista de costo de Combustible";
                        dg.EntidadAnterior = db.CostoCombustibles.Single(pc => pc.ID == Convert.ToInt32(gvData.GetFocusedRowCellValue(colID)));
                        dg.Owner = this;
                        dg.ShowDialog();
                        FillControl();
                    }
        ...
        private void btnPrecioPorTurno_Click(object sender, EventArgs e)
        {
            try
            {
                if (gvData.FocusedRowHandle >= 0)
                {
                    Forms.Dialogs.DialogCostoPorTurno dg = new Forms.Dialogs.DialogCostoPorTurno();
                    dg.Text = "Crear costo para un Turno específico";
                    dg.Owner = this;
                    dg.ShowDialog();
                    FillControl();
                }
